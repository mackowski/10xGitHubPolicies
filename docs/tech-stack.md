...
*   **SQLite In-Memory**: Provides fast, lightweight in-memory database for integration tests. This ensures that every test run starts with a clean, consistent, and isolated database without requiring Docker, making tests run in seconds instead of minutes.
